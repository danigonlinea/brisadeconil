...
- ~~**Auditoría de XSS / sanitización** del HTML legítimo en `src/i18n/translations.ts`.~~ — **Hecho**: sanitizer allowlist verificado (tags permitidos, sin `on*`, esquemas href validados); el sanitizador es defensivo: hoy no hay valores HTML en las traducciones (0 usos de `data-i18n-html` fuera del sink), DOMPurify no necesario (2026-08-24).
